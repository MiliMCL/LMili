...
> 命令根是 `/pluginid`（不是 `/plugins`）。原因是 Paper 的 `PaperPluginsCommand` 已经占用 `/plugins`，我们注册 `/plugins` 会被 Paper 在服务端启动时覆盖。

```
/pluginid list                    列出所有身份
/pluginid info <id>               查看详情
/pluginid conflicts               列出冲突（V2 §23 最低要求）
/pluginid observe <id>            进入 OBSERVE
/pluginid enable <id>             恢复 ACTIVE
/pluginid disable <id>            禁用
```
