...
@ConfigClassInfo(
        ...
        name = "auto_update",
        comments = """
                Checks GitHub Releases for newer Lmili jars on a schedule.
                Downloads are staged under auto_update/lmili and written to auto_update/core.path,
                which Hyacinthusclip can consume on the next restart.
                If target_jar_path is set, Lmili will also try to replace that launcher jar directly."""
)
public class AutoUpdateConfig implements IConfigModule {
    @ConfigInfo(name = "enabled", comments = "Whether Lmili should check for updates automatically.")
    ...
    @ConfigInfo(name = "target_jar_path", comments = """
            ...
            Leave this blank to keep the downloaded jar staged in auto_update/lmili
