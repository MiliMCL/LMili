...
public class ChunkSectionInventoryEntityTracker extends ChunkSectionEntityMovementTracker {
    ...
    public void unregister() {
        this.userCount--;
        if (this.userCount <= 0) {
            // this.level.getCurrentWorldData().containerEntityMovementTrackerMap.remove(this.sectionKey); // Mili - mixin removed
        }
    }
    ...
    private static @NotNull ChunkSectionInventoryEntityTracker registerAt(long key, Level level) {
        // Mili - containerEntityMovementTrackerMap mixin removed; create new tracker directly
        ChunkSectionInventoryEntityTracker tracker = new ChunkSectionInventoryEntityTracker(key, level);
        tracker.register();
        return tracker;
    }
