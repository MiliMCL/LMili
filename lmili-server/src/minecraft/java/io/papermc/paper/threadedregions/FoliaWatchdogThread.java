    ...
    public void run() {
        for (;;) {
            ...
            for (final RunningTick tick : ticks) {
                ...
                if (elapsed <= TimeUnit.MILLISECONDS.toNanos(fun.bm.mili.config.modules.misc.FoliaWatchdogConfig.tickRegionTimeOutMs)) { // Lmili - Add config for watchdog timeout
                    continue;
                }
                tick.lastPrint = now;
