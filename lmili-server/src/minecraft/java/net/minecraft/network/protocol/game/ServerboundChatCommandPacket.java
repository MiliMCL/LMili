...
public record ServerboundChatCommandPacket(String command) implements Packet<ServerGamePacketListener> {
    ...
    private ServerboundChatCommandPacket(final FriendlyByteBuf input) {
        this(fun.bm.mili.lmili.config.modules.fixes.LongCommandSupportConfig.enabled ? input.readUtf() : input.readUtf(MAX_CHAT_PACKET_INPUT_SIZE)); // Paper - limit chat command inputs // Luminol - add support for long command inputs
    }
