...
public class ClientboundCommandsPacket implements Packet<ClientGamePacketListener> {
    ...
    private record Entry(ClientboundCommandsPacket.@Nullable NodeStub stub, int flags, int redirect, int[] children) {
        public void write(final FriendlyByteBuf output) {
            ...
            if ((this.flags & ClientboundCommandsPacket.FLAG_REDIRECT) != 0) {
        ...
        public boolean canBuild(final IntSet unbuiltNodes) {
            return (this.flags & ClientboundCommandsPacket.FLAG_REDIRECT) == 0 || !unbuiltNodes.contains(this.redirect);
        }
    ...
    private static class NodeResolver<S> {
        ...
        public CommandNode<S> resolve(final int index) {
            ...
            if (entry.stub == null) {
                ...
                if ((entry.flags & ClientboundCommandsPacket.FLAG_REDIRECT) != 0) {
                    resultBuilder.redirect(this.resolve(entry.redirect));
                }

                boolean isExecutable = (entry.flags & ClientboundCommandsPacket.FLAG_EXECUTABLE) != 0;
                boolean isRestricted = (entry.flags & ClientboundCommandsPacket.FLAG_RESTRICTED) != 0;
