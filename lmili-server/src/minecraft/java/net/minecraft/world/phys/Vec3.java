...
public class Vec3 implements Position {
    ...
    public boolean equals(final Object o) {
        return this == o
            || o instanceof Vec3 vec3 && Double.compare(vec3.x, this.x) == 0 && Double.compare(vec3.y, this.y) == 0 && Double.compare(vec3.z, this.z) == 0;
    }
    ...
    public static Vec3 directionFromRotation(final float rotX, final float rotY) {
        float yCos = Mth.cos(-rotY * Mth.DEG_TO_RAD - Mth.PI);
        float ySin = Mth.sin(-rotY * Mth.DEG_TO_RAD - Mth.PI);
        float xCos = -Mth.cos(-rotX * Mth.DEG_TO_RAD);
        float xSin = Mth.sin(-rotX * Mth.DEG_TO_RAD);
        return new Vec3(ySin * xCos, xSin, yCos * xCos);
    }

    public Vec2 rotation() {
        float yaw = (float)Math.atan2(-this.x, this.z) * Mth.RAD_TO_DEG;
        float pitch = (float)Math.asin(-this.y / Math.sqrt(this.x * this.x + this.y * this.y + this.z * this.z)) * Mth.RAD_TO_DEG;
