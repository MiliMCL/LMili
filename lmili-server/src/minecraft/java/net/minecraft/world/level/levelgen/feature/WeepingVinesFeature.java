...
public class WeepingVinesFeature extends Feature<NoneFeatureConfiguration> {
    ...
    private void placeRoofNetherWart(final LevelAccessor level, final RandomSource random, final BlockPos origin) {
        level.setBlock(origin, Blocks.NETHER_WART_BLOCK.defaultBlockState(), Block.UPDATE_CLIENTS);
        ...
        for (int i = 0; i < 200; i++) {
            ...
            if (level.isEmptyBlock(placePos)) {
                ...
                if (neighbours == 1) {
                    level.setBlock(placePos, Blocks.NETHER_WART_BLOCK.defaultBlockState(), Block.UPDATE_CLIENTS);
                }
            }
        }
    }
        ...
        for (int height = 0; height <= totalHeight; height++) {
            if (level.isEmptyBlock(placePos)) {
                if (height == totalHeight || !level.isEmptyBlock(placePos.below())) {
                    level.setBlock(
                        placePos,
                        Blocks.WEEPING_VINES.defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(random, minAge, naxAge)),
