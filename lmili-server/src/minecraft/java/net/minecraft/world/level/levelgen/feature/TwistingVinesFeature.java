        ...
        for (int height = 1; height <= totalHeight; height++) {
            if (level.isEmptyBlock(placePos)) {
                if (height == totalHeight || !level.isEmptyBlock(placePos.above())) {
                    level.setBlock(
                        placePos,
                        Blocks.TWISTING_VINES.defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Mth.nextInt(random, minAge, naxAge)),
                        Block.UPDATE_CLIENTS
                    );
                    break;
                }

                level.setBlock(placePos, Blocks.TWISTING_VINES_PLANT.defaultBlockState(), Block.UPDATE_CLIENTS);
            }
