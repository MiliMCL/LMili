...
public class StateDefinition<O, S extends StateHolder<O, S>> {
    ...
    private static <O, S extends StateHolder<O, S>> ImmutableList<S> initStateTables(final ImmutableList<S> states) {
        if (!states.isEmpty()) {
            ((ca.spottedleaf.moonrise.patches.blockstate_propertyaccess.PropertyAccessStateHolder<O, S>)(StateHolder<O, S>)states.get(0)).moonrise$init(states);
        }
        return states;
    }
    ...
    private static <O, S extends StateHolder<O, S>> ImmutableList<S> createSingletonState(final O owner, final StateDefinition.Factory<O, S> factory) {
        ...
        return initStateTables(ImmutableList.of(singletonState));  // Paper - optimise blockstate property access
    }
    ...
    private static <O, S extends StateHolder<O, S>, T extends Comparable<T>> ImmutableList<S> createSinglePropertyStates(
        ...
        return initStateTables(states.build());  // Paper - optimise blockstate property access
    }
