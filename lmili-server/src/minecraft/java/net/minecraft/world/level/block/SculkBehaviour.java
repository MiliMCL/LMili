    ...
    SculkBehaviour DEFAULT = new SculkBehaviour() {
        ...
        public boolean attemptSpreadVein(
            ...
            if (facings == null) {
                ...
                return !facings.isEmpty()
                    ? (state.isAir() || state.getFluidState().is(Fluids.WATER)) && SculkVeinBlock.regrow(level, pos, state, facings)
                    : SculkBehaviour.super.attemptSpreadVein(level, pos, state, facings, postProcess);
            }
        }
