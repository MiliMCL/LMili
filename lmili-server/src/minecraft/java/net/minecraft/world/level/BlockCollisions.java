...
public class BlockCollisions<T> extends AbstractIterator<T> {
    ...
    protected T computeNext() {
        while (this.cursor.advance()) {
            ...
            if (cursorFaceType != Cursor3D.TYPE_CORNER) {
                ...
                if (chunk != null) {
                    ...
                    if ((!this.onlySuffocatingBlocks || blockState.isSuffocating(chunk, this.pos))
                        && (cursorFaceType != Cursor3D.TYPE_FACE || blockState.hasLargeCollisionShape())
                        && (cursorFaceType != Cursor3D.TYPE_EDGE || blockState.is(Blocks.MOVING_PISTON))) {
