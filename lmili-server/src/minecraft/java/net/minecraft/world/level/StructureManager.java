...
public class StructureManager {
    ...
    public List<StructureStart> startsForStructure(final ChunkPos pos, final Predicate<Structure> matcher) {
        Map<Structure, LongSet> allReferences = this.level.getChunk(pos.x(), pos.z(), ChunkStatus.STRUCTURE_REFERENCES).getAllReferences(); // Folia - region threading
    ...
    public StructureStart getStructureWithPieceAt(final BlockPos blockPos, final HolderSet<Structure> structures) {
        return this.getStructureWithPieceAt(blockPos, structures::contains); // Folia - region threading
    }
    ...
    public StructureStart getStructureWithPieceAt(final BlockPos blockPos, final Predicate<Holder<Structure>> predicate) {
        ...
        for (StructureStart structureStart : this.startsForStructure(
            ChunkPos.containing(blockPos), s -> structures.get(structures.getId(s)).map(predicate::test).orElse(false) // Paper - Fix swamp hut cat generation deadlock // Folia - region threading
        )) {
    ...
    public void addReference(final StructureStart start) {
        //start.addReference(); // Folia - region threading - move to caller
