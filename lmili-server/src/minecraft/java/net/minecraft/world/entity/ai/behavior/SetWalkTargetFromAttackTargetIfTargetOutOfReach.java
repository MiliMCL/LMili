    ...
    public static BehaviorControl<Mob> create(final Function<LivingEntity, Float> speedModifier) {
        return BehaviorBuilder.create(
            i -> i.group(
                ...
                .apply(
                    i,
                    (walkTarget, lookTarget, attackTarget, nearestEntities) -> (level, body, timestamp) -> {
                        LivingEntity toAttack = i.get(attackTarget);
                        Optional<NearestVisibleLivingEntities> entities = i.tryGet(nearestEntities);
                        if (entities.isPresent()
                            && entities.get().contains(toAttack)
                            && BehaviorUtils.isWithinAttackRange(body, toAttack, PROJECTILE_ATTACK_RANGE_BUFFER)) {
                            walkTarget.erase();
                        } else {
