...
public class RandomStroll {
    ...
    public static OneShot<PathfinderMob> stroll(final float speedModifier, final boolean mayStrollFromWater) {
        return strollFlyOrSwim(speedModifier, body -> LandRandomPos.getPos(body, MAX_XZ_DIST, MAX_Y_DIST), mayStrollFromWater ? b -> true : b -> !b.isInWater());
    }
    ...
    public static BehaviorControl<PathfinderMob> fly(final float speedModifier) {
        return strollFlyOrSwim(speedModifier, body -> getTargetFlyPos(body, MAX_XZ_DIST, MAX_Y_DIST), b -> true);
    }
