...
public interface MemoryCondition<F extends K1, Value> {
    ...
    record Absent<Value>(@Override MemoryModuleType<Value> memory) implements MemoryCondition<Mu<Unit>, Value> {
        @Override
    ...
    record Present<Value>(@Override MemoryModuleType<Value> memory) implements MemoryCondition<com.mojang.datafixers.kinds.IdF.Mu, Value> {
        @Override
        public MemoryStatus condition() {
    ...
    record Registered<Value>(@Override MemoryModuleType<Value> memory) implements MemoryCondition<com.mojang.datafixers.kinds.OptionalBox.Mu, Value> {
        @Override
        public MemoryStatus condition() {
