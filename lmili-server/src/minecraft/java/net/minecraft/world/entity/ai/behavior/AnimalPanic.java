...
public class AnimalPanic<E extends PathfinderMob> extends Behavior<E> {
    ...
    public AnimalPanic(final float speedMultiplier) {
        this(speedMultiplier, mob -> DamageTypeTags.PANIC_CAUSES, mob -> LandRandomPos.getPos(mob, PANIC_DISTANCE_HORIZONTAL, PANIC_DISTANCE_VERTICAL));
    }
    ...
    public AnimalPanic(final float speedMultiplier, final int flyHeight) {
        this(
            ...
            mob -> AirAndWaterRandomPos.getPos(
                mob, PANIC_DISTANCE_HORIZONTAL, PANIC_DISTANCE_VERTICAL, flyHeight, mob.getViewVector(0.0F).x, mob.getViewVector(0.0F).z, (float) (Math.PI / 2)
            )
        );
    }

    public AnimalPanic(final float speedMultiplier, final Function<PathfinderMob, TagKey<DamageType>> panicCausingDamageTypes) {
        this(speedMultiplier, panicCausingDamageTypes, mob -> LandRandomPos.getPos(mob, PANIC_DISTANCE_HORIZONTAL, PANIC_DISTANCE_VERTICAL));
    }
