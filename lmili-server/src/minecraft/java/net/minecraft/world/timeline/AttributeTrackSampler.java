...
public class AttributeTrackSampler<Value, Argument> implements EnvironmentAttributeLayer.TimeBased<Value> {
    ...
    private AttributeTrackSampler(
            final Holder<WorldClock> clock,
            final AttributeModifier<Value, Argument> modifier,
            final KeyframeTrackSampler<Argument> argumentSampler,
            final ClockManager clockManager
    ) {
        this.clock = clock;
        this.modifier = modifier;
        this.argumentSampler = argumentSampler;
        this.clockManager = clockManager;
    }

    @Override
    public AttributeTrackSampler<Value, Argument> copy() {
        return new AttributeTrackSampler<>(this.clock, this.modifier, this.argumentSampler, this.clockManager);
    }
