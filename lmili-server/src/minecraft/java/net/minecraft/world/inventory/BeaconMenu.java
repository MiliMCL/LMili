...
public class BeaconMenu extends AbstractContainerMenu {
    ...
    public boolean updateEffects(final Optional<Holder<MobEffect>> primary, final Optional<Holder<MobEffect>> secondary) {
        if (this.paymentSlot.hasItem()) {
            ...
            if (!event.callEvent()) {
                return true; // don't disconnect player when the event is cancelled
            }

            this.beaconData.set(1, encodeEffect(event.getPrimary() == null ? null : org.bukkit.craftbukkit.potion.CraftPotionEffectType.bukkitToMinecraftHolder(event.getPrimary())));
            this.beaconData.set(2, encodeEffect(event.getSecondary() == null ? null : org.bukkit.craftbukkit.potion.CraftPotionEffectType.bukkitToMinecraftHolder(event.getSecondary())));
