...
public record OminousBottleAmplifier(int value) implements ConsumableListener, TooltipProvider {
    ...
    public void onConsume(final Level level, final LivingEntity user, final ItemStack stack, final Consumable consumable) {
        user.addEffect(new MobEffectInstance(MobEffects.BAD_OMEN, EFFECT_DURATION, this.value, false, false, true)); // Paper - properly resend entities - diff on change for below
    }
    ...
    public void addToTooltip(
        ...
        List<MobEffectInstance> effects = List.of(new MobEffectInstance(MobEffects.BAD_OMEN, EFFECT_DURATION, this.value, false, false, true));
