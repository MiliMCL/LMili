...
public class LevelTicks<T> implements LevelTickAccess<T> {
    ...
    private final BiConsumer<LevelChunkTicks<T>, ScheduledTick<T>> chunkScheduleUpdater = (container, newTick) -> {
        if (newTick.equals(container.peek())) { // Folia - diff on change
            this.updateContainerScheduling(newTick); // Folia - diff on change
        }
    };
    ...
    public LevelTicks(final LongPredicate tickCheck, final net.minecraft.server.level.ServerLevel world, final boolean isBlock) { this.world = world; this.isBlock = isBlock; // Folia - add world and isBlock
    ...
    public void addContainer(final ChunkPos pos, final LevelChunkTicks<T> container) {
        ...
        container.setOnTickAdded((LevelChunkTicks<T> levelChunkTicks, ScheduledTick<T> tick) -> {
