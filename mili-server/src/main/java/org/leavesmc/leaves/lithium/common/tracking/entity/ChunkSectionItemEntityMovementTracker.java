...
public class ChunkSectionItemEntityMovementTracker extends ChunkSectionEntityMovementTracker {
    public ChunkSectionItemEntityMovementTracker(long sectionKey, Level level) {
    ...
    public void unregister() {
        this.userCount--;
        if (this.userCount <= 0) {
            // this.level.getCurrentWorldData().itemEntityMovementTrackerMap.remove(this.sectionKey); // Mili - mixin removed
        }
    }
    ...
    private static @NotNull ChunkSectionItemEntityMovementTracker registerAt(long key, Level level) {
        // Mili - itemEntityMovementTrackerMap mixin removed; create new tracker directly
        ChunkSectionItemEntityMovementTracker tracker = new ChunkSectionItemEntityMovementTracker(key, level);
        tracker.register();
        return tracker;
    }
